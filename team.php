...
if($login_user_id){
	...
	$check_collect = DB::LimitQuery('collect', array(
				'condition' => array(
					'team_id' => $id,
...
if($act == 'collect'){
	if(!$login_user_id){
		...
		if($is_collect != 1){
			...
			$insert = array('team_id','gid','user_id','title','time','team_price','image');
...
foreach($comment_all_list as $key=>$val){
	   ...
	   $comment_all_list[$key]['username'] =  $res['username'];
...
if($team['group_id']){
    $gid_arr = Table::Fetch('category', $team['group_id']);
	$home_url .= '<a href="/category.php?gid='.$team[group_id].'">'.$gid_arr[name].'</a> >';
}
if($team['sub_id']){	
   $sub_id_arr = Table::Fetch('category', $team['sub_id']);
   $home_url .= '<a href="/category.php?gid='.$team[group_id].'&sid='.$team[sub_id].'">'.$sub_id_arr[name].'</a> >';
}
if($team['sub_ids']){
   $sub_ids_arr = Table::Fetch('category', $team['sub_ids']);	
   $home_url .= '<a href="/category.php?gid='.$team[group_id].'&sid='.$team[sub_id].'&sids='.$team[sub_ids].'">'.$sub_ids_arr[name].'</a> >';
}
...
$tc_team = DB::LimitQuery('team', array(
...
if ($team['team_type'] == 'collect') {
	die(include template('team_view_collect'));
}

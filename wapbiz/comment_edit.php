...
if($act == 3){
   $condition['comment_grade_three'] = 3;
}elseif($act == 2){
   $condition['comment_grade_three'] = 2;
}elseif($act == 1){
   $condition['comment_grade_three'] = 1;
}
...
foreach($comment_list as $key=>$val){
   ...
   $comment_list[$key]['comment_time'] = $val['add_time'];
   $res = get_com_user($val['user_id']);
   $comment_list[$key]['username'] =  '***'.substr($res['username'],1);
}

	...
	var a = {
		...
		_bindClick: function() {
			...
			c.stars.on("click.raty", function(e) {
				...
				c.score.val((c.opt.half || c.opt.precision) ? d.data("score") : alt);
				if (c.opt.click) {
		...
		_bindOver: function() {
			...
			c.stars.on(e, function(g) {
                var h = 0;
                if (c.opt.is_div) {
                    h = parseInt($(this).attr("alt"), 10);
                } else {
                    h = parseInt(this.alt, 10);
                }
				if (c.opt.half) {
		...
		readOnly: function(c) {
			return this.each(function() {
				...
				if (d.data("readonly") !== c) {
					if (c) {
                        if(this.opt.is_div) {
                            d.off(".raty").children("div").off(".raty");
                        } else {

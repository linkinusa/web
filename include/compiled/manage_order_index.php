		...
		<div class="box clear">
            ...
            <div class="box-content">
				...
				<div class="sect" style="padding:0 10px;">
					<form method="get">
						<p style="margin:5px 0; overflow:hidden">订单编号：<input type="text" name="id" value="<?php echo htmlspecialchars($id); ?>" class="h-input"/>&nbsp;用户：<input type="text" name="uemail" class="h-input" value="<?php echo htmlspecialchars($uemail); ?>" >&nbsp;项目编号：<input type="text" name="team_id" class="h-input number" value="<?php echo $team_id; ?>" ></p>
						<p style="margin:5px 0;overflow:hidden">下单日期：<input type="text" class="h-input" onFocus="WdatePicker({isShowClear:false,readOnly:true})" name="cbday" value="<?php echo $cbday; ?>" /> - <input type="text" class="h-input" onFocus="WdatePicker({isShowClear:false,readOnly:true})" name="ceday" value="<?php echo $ceday; ?>" />&nbsp;付款日期：<input type="text" class="h-input" onFocus="WdatePicker({isShowClear:false,readOnly:true})" name="pbday" value="<?php echo $pbday; ?>" /> - <input type="text" class="h-input" onFocus="WdatePicker({isShowClear:false,readOnly:true})" name="peday" value="<?php echo $peday; ?>" /></p>
						<p style="margin:5px 0;overflow:hidden"><input type="submit" value="筛选" class="formbutton"  style="padding:1px 6px;"/></p>
					<form>
				</div>
                <div class="sect">
					<table id="orders-list" cellspacing="0" cellpadding="0" border="0" class="coupons-table">
					<tr><th width="30">ID</th><th width="40">父ID</th><th width="340">项目</th><th width="150">用户</th><th width="40" nowrap>数量</th><th width="50" nowrap>总款</th><th width="50" nowrap>余付</th><th width="50" nowrap>支付</th><th width="50" nowrap>递送</th><th width="50" nowrap>操作</th></tr>
					<?php if(is_array($orders)){foreach($orders AS $index=>$one) { ?>
					<tr <?php echo $index%2?'':'class="alt"'; ?> id="order-list-id-<?php echo $one['id']; ?>">
						<td><?php echo $one['id']; ?></td>
                        <td>
                        <?php if($one['packageid'] != -8){?>
                         <?php echo $one['packageid']; ?>
                        <?php } else { ?>
                         <?php echo $one['id']; ?>
                        <?php }?>
                        </td>

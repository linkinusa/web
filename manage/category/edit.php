...
$category = $ecategory = Table::Fetch('category', $id);
$category = $_POST;
$category['icon'] = upload_image('icon',$ecategory['icon'],'team');
$category['thumb'] = upload_image('thumb',$ecategory['thumb'],'team');

$table = new Table('category', $category);
$table->letter = strtoupper($table->letter);
$uarray = array( 'zone', 'ename', 'letter', 'name', 'czone','fid', 'display','is_hot', 'sort_order', 'relate_data','icon','thumb'); 
